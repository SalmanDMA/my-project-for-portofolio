...
const PlaneFiguresDetailInputParallelogram = () => {
 ...
 const handleSubmit = (e) => {
  e.preventDefault();
  const { a, b, t } = inputValues;
  const luasResult = hitungLuas(parseFloat(a), parseFloat(b));
  const kelilingResult = hitungKeliling(parseFloat(a), parseFloat(t));
  ...
  if (editById !== null) {
   ...
   setData([...data, { id: data.length + 1, a, b, t, luas: luasResult, keliling: kelilingResult }]);
  }
  setInputValues({ a: '', b: '', t: '' });
 };
 ...
 const handleEdit = (id) => {
  ...
  setInputValues({ a: editData.a, b: editData.b, t: editData.t });
  setEditById(id);
 };
...
export default PlaneFiguresDetailInputParallelogram;
