...
const PlaneFiguresDetailInputTriangle = () => {
 ...
 const handleSubmit = (e) => {
  e.preventDefault();
  const { a, b, c, t } = inputValues;
  const luasResult = hitungLuas(parseFloat(a), parseFloat(t));
  const kelilingResult = hitungKeliling(parseFloat(a), parseFloat(b), parseFloat(c));
  ...
  if (editById !== null) {
   ...
   setData([...data, { id: data.length + 1, a, b, c, t, luas: luasResult, keliling: kelilingResult }]);
  }
  setInputValues({ a: '', b: '', c: '', t: '' });
 };

 const hitungLuas = (a, t) => {
  return (a * t) / 2;
 };
 ...
 const handleEdit = (id) => {
  ...
  setInputValues({ a: editData.a, b: editData.b, c: editData.c, t: editData.t });
  setEditById(id);
 };
 ...
 return (
  <div className='w-100 py-5 d-flex flex-lg-row flex-column gap-5'>
   <form className='form-container d-flex flex-column gap-3' onSubmit={handleSubmit}>
    ...
    <div className='mb-3 d-flex flex-column w-100 '>
     <h3 className='my-font-verdana fw-bold'>Enter c value</h3>
     <input type='number' className='form-control' placeholder='Type here' value={inputValues.c} onChange={(e) => setInputValues({ ...inputValues, c: e.target.value })} required />
    </div>
    <div className='mb-3 d-flex flex-column w-100 '>
...
export default PlaneFiguresDetailInputTriangle;
